            ...
            var ExplorerBaseView = (function (_super) {
                                ...
                                if (process.platform === 'win32') {
                                    ...
                                    me.remoteAutoBtPanConnector.on('connectableDenebCountChanged', function (connectableDenebCount) {
                                        if (connectableDenebCount > 1) {
                                            var errOptions = {
                                                title: $.i18n.t('dialog.notice.foundMultiBtDps.title'),
                                                message: $.i18n.t('dialog.notice.foundMultiBtDps.message'),
                                                type: 'none',
                                            };
                                            var activeWindow = Explorer.Handler.subWindowHandler.getActiveSubWindow();
                                            if (activeWindow && activeWindow.getDialogName() === View.Dialog.DialogName.WINDOW_SETTING) {
                                                activeWindow.error(errOptions);
                                            }
                                            else {
                                                View.Dialog.openErrorDialog(errOptions, function (response) { });
                                            }
                                        }
                                    });
                ...
                ExplorerBaseView.prototype.onClose = function (browserWindow) {
                    if (this.isOperationRunning()) {
                        var dialogOptions = {
                            title: $.i18n.t('dialog.confirm.abortExit.title'),
                            message: $.i18n.t('dialog.confirm.abortExit.message'),
                        };
                        var self_1 = this;
                        DPMW.View.Dialog.openYesNoDialog(dialogOptions, function (response) {
                            ...
                            self_1.cancelRunningOperations(function (error) {
                ...
                ExplorerBaseView.prototype.onForceClose = function (browserWindow) {
                    if (this.isOperationRunning()) {
                        ...
                        this.cancelRunningOperations(function (error) {
                            self_2.closeWindow(browserWindow);
                        });
                        return;
                    }
                    this.closeWindow(browserWindow);
