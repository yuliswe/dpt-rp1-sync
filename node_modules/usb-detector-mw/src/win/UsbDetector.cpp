...
void UsbDetector::OnDeviceAdded(
	...
	try {
		...
		errMsg = UsbDeviceFinder::FindUsbDevices(sPnpDeviceId, srcDeviceList);
		if (!errMsg.empty()) {
			LOG_ERROR(_T("UsbDeviceFinder::FindUsbDevices(%s) failed. [%S]"), sPnpDeviceId.c_str(), errMsg.c_str());
			throw E_FAIL;
		}
